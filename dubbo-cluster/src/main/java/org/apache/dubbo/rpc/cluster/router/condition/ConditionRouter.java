...
public class ConditionRouter extends AbstractRouter {
    // 基于一些你自定义的条件，去进行router路由，如果一个invoker是符合条件的，那么就可以返回出来去进行调用
    ...
    public void init(String rule) {
        try {
            ...
            // dubbo的condition路由规则的使用语法
            // host = xxxx => host = xxx
            // 前面的这串东西叫做，when条件，当满足某些条件的情况下，此时when条件
            // 后面的这串东西叫做，then条件，when条件满足之后，走then条件，此时会如何如何
    ...
    private static Map<String, MatchPair> parseRule(String rule)
        ...
        // 他想做的事情，就是对你的类似于host = xxx,xxx
        // 在对你的字符串做一个解析，把你的规则语法里的各种东西都提取出来，做一个提取出来的东西放到各种list、map里去
        // 这样的话，就可以通过list和map，访问你的各个规则语法
    ...
    public <T> List<Invoker<T>> route(List<Invoker<T>> invokers, URL url, Invocation invocation)
        ...
        try {
            // matchWhen，如果没有达到matchWhen的条件
            // 如果invokers列表里，没有一个人是符合when条件的，直接就用原来的invokers就可以了
            ...
            for (Invoker<T> invoker : invokers) {
                if (matchThen(invoker.getUrl(), url)) {
                    result.add(invoker); // 就可以把invoker，匹配上了when和then的条件之后
                }
            }
            if (!result.isEmpty()) {
                return result;
...
//        return true;
    ...
    private boolean matchCondition(Map<String, MatchPair> condition, URL url, URL param, Invocation invocation) {
        ...
        for (Map.Entry<String, MatchPair> matchPair : condition.entrySet()) {
            ...
            // 关于具体的一些rule路由条件的内容，就不在这里讲了，涉及的是一些使用层面
